...
Please note you will also have to have a H2 database running. Hint install database and execute the java binary with the following command java -jar h2-1.4.xxx.jar 

Enter the following URL in a browser

    http://localhost:9000

The login page should be presented. The application has no authentication configured for this version, so by entering any username and password, the dashboard should be displayed. The angular client is integrated with the Play backend and any actions on the Angular client will be routed to the Play backend
